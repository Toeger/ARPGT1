	...
	for (float dx : testnumbers) {
		for (float dy : testnumbers) {
			for (float radius : {1.f, 0.1f, 100.f}) {
				assert_debug(Physical::Helper::collides({dx, dy}, {dx, dy}, radius));
				assert_debug(Physical::Helper::collides({dx + radius * 0.8f, dy}, {dx, dy}, radius));
				assert_debug(Physical::Helper::collides({dx - radius * 0.8f, dy}, {dx, dy}, radius));
				assert_debug(Physical::Helper::collides({dx, dy + radius * 0.8f}, {dx, dy}, radius));
				assert_debug(Physical::Helper::collides({dx, dy + radius * 0.8f}, {dx, dy}, radius));
				for (float sign1 : {1, -1}) {
					for (float sign2 : {1, -1}) {
						assert_debug(Physical::Helper::collides({dx + sign1 * radius * 0.7f, dy + sign2 * radius * 0.7f}, {dx, dy}, radius));
						assert_debug(!Physical::Helper::collides({dx + sign1 * radius * 1.1f, dy}, {dx, dy}, radius));
						assert_debug(!Physical::Helper::collides({dx, dy + sign1 * radius * 1.1f}, {dx, dy}, radius));
						assert_debug(!Physical::Helper::collides({dx + sign1 * radius * 0.8f, dy + sign2 * radius * 0.8f}, {dx, dy}, radius));
					}
				}
			}
		}
	...
	for (float x : testnumbers) {
		for (float y : testnumbers) {
			for (float d : {0.1f, 1.f, 10.f, 33.f, 444.f}) {
				assert_debug(Physical::Helper::collides({x, y}, {x - d, x + d, y - d, y + d}));
				assert_debug(!Physical::Helper::collides({x, y}, {x + d, x + 2 * d, y - d, y + d}));
				assert_debug(!Physical::Helper::collides({x, y}, {x - 2 * d, x - d, y - d, y + d}));
				assert_debug(!Physical::Helper::collides({x, y}, {x - d, x + d, y + d, y + 2 * d}));
				assert_debug(!Physical::Helper::collides({x, y}, {x - d, x + d, y - 2 * d, y - d}));
			}
		}
	}
}
...
static void test_circle_circle_collisions() {
	...
	assert_debug(Physical::collides(c1, t1, c2, t2));
	t1.vector.x = 25;
	assert_debug(Physical::collides(c1, t1, c2, t2));
	t1.vector.x = -25;
	assert_debug(Physical::collides(c1, t1, c2, t2));
...
static void test_line_line_collisions() {
	...
	assert_debug(Physical::collides(l1, t1, l2, t2));
	l1.vector.x = 10;
	l2.vector.x = 10;
	assert_debug(Physical::collides(l1, t1, l2, t2));
	t1.vector.y = 1;
...
static void test_line_circle_collision() {
	...
	Physical::Circle c(1);
	Physical::Transformator t1({}, {}), t2({}, {});
	assert_debug(Physical::collides(c, t1, l, t2));
	t1.vector.x = 2;
	t1.vector.y = 2;
	assert_debug(!Physical::collides(c, t1, l, t2));
	t1.vector.y = 2;
	...
	for (auto &t : transformators) {
		...
		t2.vector.x = -2;
		t2.vector.y = -2;
		assert_debug(!Physical::collides(l, t + t1, c, t + t2));
		t2.vector.x = 0;
		t2.vector.y = -4;
		assert_debug(!Physical::collides(l, t + t1, c, t + t2));
		t2.vector.x = 1;
		...
		for (auto y : testnumbers) {
			for (auto dx : testnumbers) {
				for (auto dy : testnumbers) {
					...
					assert_debug(Physical::collides(cicle, t + t1, rect, t + t2));
